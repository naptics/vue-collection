                    ...
                    <tr>
                        {props.headings.map(heading => (
                            <th
                                key={heading.key}
                                scope="col"
                                class="table-padding text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                            >
                                {heading.label}
                            </th>
                        ))}
                ...
                {props.items.length > 0 && (
                    <tbody>
                        {props.items.map((item, itemIndex) => (
                            <tr key={itemIndex} class={itemIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                {props.headings.map(heading => (
                                    <td key={itemIndex + '-' + heading.key} class="table-padding text-sm text-gray-500">
                                        <div
                                            class={[heading.emph ? 'font-medium text-gray-900' : '', heading.cellClass]}
                                        >
